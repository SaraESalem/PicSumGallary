...
extension UIImageView {
    ...
    func setImageWithUrlStringToCashe(urlStr: String,placholder: UIImage? = nil,completionHandler:@escaping(UIImage?) -> Void) {
        let url = URL(string: urlStr.vaildStringUrl())
        self.kf.setImage(with: url, placeholder: placholder, options: nil, progressBlock: nil) {   result in
            switch result {
            case .success(let value):
